...
// The engines prism provides to NVGT, mapped to their long standing NVGT names for script compatibility.
// Entries are only registered when the connected prism build actually contains the backend, so the same table works everywhere: windows picks up sapi5 and onecore today, linux picks up speechd and spiel as soon as its prism build includes those backends.
...
// prism 0.16.5's UIA backend initializes as soon as the process owns a qualifying top level window and reports runtime support whenever the automation COM class exists, neither of which requires an actual screen reader to be listening, so without this check it silently swallows speech whenever the user's reader is gone. This is the same probe newer prism versions perform inside the backend itself.
static bool sr_uia_reader_listening() {
	BOOL flag = FALSE;
...
static void sr_release_backend() {
...
// Attempts to select the highest priority screen reader backend that initializes, in the registry's own priority order. Must be called with g_sr_mutex held.
static bool sr_select_backend() {
	sr_release_backend();
	PrismContext *ctx = prism_get_context();
	if (!ctx) return false;
...
std::string screen_reader_detect() {
	lock_guard<mutex> lock(g_sr_mutex);
	if (!g_sr_backend && !sr_select_backend()) return "";
	const char *name = prism_backend_name(g_sr_backend);
	return name? std::string(name) : std::string();
}
bool screen_reader_has_speech() {
	lock_guard<mutex> lock(g_sr_mutex);
	if (!g_sr_backend && !sr_select_backend()) return false;
	return prism_backend_get_features(g_sr_backend) & PRISM_BACKEND_SUPPORTS_SPEAK;
}
bool screen_reader_has_braille() {
	lock_guard<mutex> lock(g_sr_mutex);
	if (!g_sr_backend && !sr_select_backend()) return false;
	return prism_backend_get_features(g_sr_backend) & PRISM_BACKEND_SUPPORTS_BRAILLE;
}
bool screen_reader_is_speaking() {
	lock_guard<mutex> lock(g_sr_mutex);
	if (!g_sr_backend && !sr_select_backend()) return false;
	if (!(prism_backend_get_features(g_sr_backend) & PRISM_BACKEND_SUPPORTS_IS_SPEAKING)) return false;
	bool speaking = false;
...
bool screen_reader_output(const std::string& text, bool interrupt) {
	lock_guard<mutex> lock(g_sr_mutex);
	if ((!g_sr_backend && !sr_select_backend()) || text.empty()) return false;
	PrismError err = prism_backend_output(g_sr_backend, text.c_str(), interrupt);
	if (sr_error_invalidates_backend(err)) sr_release_backend();
	return err == PRISM_OK;
}
bool screen_reader_speak(const std::string& text, bool interrupt) {
	lock_guard<mutex> lock(g_sr_mutex);
	if ((!g_sr_backend && !sr_select_backend()) || text.empty()) return false;
	PrismError err = prism_backend_speak(g_sr_backend, text.c_str(), interrupt);
	if (sr_error_invalidates_backend(err)) sr_release_backend();
	return err == PRISM_OK;
}
bool screen_reader_braille(const std::string& text) {
	lock_guard<mutex> lock(g_sr_mutex);
	if ((!g_sr_backend && !sr_select_backend()) || text.empty()) return false;
	PrismError err = prism_backend_braille(g_sr_backend, text.c_str());
	if (sr_error_invalidates_backend(err)) sr_release_backend();
	return err == PRISM_OK;
}
bool screen_reader_silence() {
	...
	if (sr_error_invalidates_backend(err)) sr_release_backend();
	return err == PRISM_OK || err == PRISM_ERROR_NOT_SPEAKING || err == PRISM_ERROR_NOT_IMPLEMENTED;
}
